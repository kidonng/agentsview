...
func TestParseQwenPawSession_BasicUserAssistant(t *testing.T) {
	...
	assert.Equal("你好", sess.FirstMessage)
	...
	assert.Equal("你好", msgs[0].Content)
	assert.Equal(RoleAssistant, msgs[1].Role)
	assert.Equal("你好，有什么可以帮你的？", msgs[1].Content)
}
...
func TestParseQwenPawSession_ThinkingExtracted(t *testing.T) {
	...
	assert.True(msgs[1].HasThinking, "HasThinking flag")
	assert.Equal("我应该先思考一下", msgs[1].ThinkingText)
	assert.Equal("答案", msgs[1].Content)
}
...
func TestParseQwenPawSession_ToolUseAndResult(t *testing.T) {
	...
	aMsg := msgs[1]
	assert.Equal(RoleAssistant, aMsg.Role)
	assert.True(aMsg.HasToolUse, "HasToolUse flag")
	require.Equal(1, len(aMsg.ToolCalls), "tool call count")
	assert.Equal("call_abc", aMsg.ToolCalls[0].ToolUseID)
	assert.Equal("read_file", aMsg.ToolCalls[0].ToolName)
	assert.Contains(aMsg.ToolCalls[0].InputJSON, "foo.txt")

	sMsg := msgs[2]
	assert.Equal(RoleUser, sMsg.Role)
	assert.True(sMsg.IsSystem, "IsSystem on tool_result carrier")
	require.Equal(1, len(sMsg.ToolResults), "tool result count")
	assert.Equal("call_abc", sMsg.ToolResults[0].ToolUseID)
	assert.Equal(len("file contents here"),
...
func TestParseQwenPawSession_MultipleToolUsesInOneMessage(t *testing.T) {
	...
	require.Equal(2, len(msgs[0].ToolCalls))
	assert.Equal("call_1", msgs[0].ToolCalls[0].ToolUseID)
	assert.Equal("call_2", msgs[0].ToolCalls[1].ToolUseID)
}
...
func TestParseQwenPawSession_SystemTextMessage(t *testing.T) {
	...
	assert.Equal("system notice", msgs[1].Content)
	assert.Equal(1, sess.UserMessageCount)
}
...
func TestDiscoverQwenPawSessions(t *testing.T) {
	...
	require.NoError(os.MkdirAll(fmDir, 0o755))
	require.NoError(os.WriteFile(
	...
	for _, f := range files {
		assert.Equal(AgentQwenPaw, f.Agent)
	}
	projects := map[string]int{}
	for _, f := range files {
		projects[f.Project]++
	}
	assert.Equal(2, projects["default"])
	assert.Equal(1, projects["fund_manager"])
}
...
func TestDiscoverQwenPawSessions_IncludesConsoleSubdir(t *testing.T) {
	...
	require.NoError(os.MkdirAll(consoleDir, 0o755))
	require.NoError(os.WriteFile(
...
func TestDiscoverQwenPawSessions_FiltersNonSessionFiles(t *testing.T) {
	...
	require.NoError(os.MkdirAll(dialogDir, 0o755))
	require.NoError(os.WriteFile(
...
func TestParseQwenPawSession_IDsDifferBySubdir(t *testing.T) {
	...
	require.NoError(os.MkdirAll(rootDir, 0o755))
	consoleDir := filepath.Join(root, "default", "sessions", "console")
	require.NoError(os.MkdirAll(consoleDir, 0o755))
	...
	assert.Equal("qwenpaw:default:foo", rootSess.ID,
		"root layout ID")
	assert.Equal("qwenpaw:default:console:foo", consoleSess.ID,
		"console subdir ID — must differ from root")
	assert.NotEqual(rootSess.ID, consoleSess.ID)
}
...
func TestFindQwenPawSourceFile_RootAndConsoleResolveIndependently(t *testing.T) {
	...
	require.NoError(os.WriteFile(rootPath, []byte(`{}`), 0o644))
	require.NoError(os.WriteFile(consolePath, []byte(`{}`), 0o644))
...
func TestFindQwenPawSourceFile_RejectsTraversal(t *testing.T) {
	...
	require.NoError(os.WriteFile(outside,
		[]byte(`{"agent":{"memory":{"content":[]}}}`), 0o644))
	...
	for _, rawID := range []string{
		...
		assert.Equal("", findQwenPawTestSourceFile(t, root, rawID),
			"rawID %q must not resolve", rawID)
	}
	for _, bad := range []string{".", ".."} {
		assert.False(IsValidQwenPawIDPart(bad),
...
func TestQwenPawFixtures(t *testing.T) {
	...
	for _, f := range files {
		...
		require.NoErrorf(err, "parse %s", f.Path)
		require.NotNil(sess)
	...
	require.Len(toolMsgs, 4)
	assert.True(toolMsgs[1].HasToolUse, "assistant message has tool_use")
	require.Len(toolMsgs[1].ToolCalls, 1)
	assert.Equal("call_aaaaaaaaaaaa", toolMsgs[1].ToolCalls[0].ToolUseID)
	resultMsg := toolMsgs[2]
	assert.Equal(RoleUser, resultMsg.Role)
	assert.True(resultMsg.IsSystem, "tool_result carrier is a system message")
	require.Len(resultMsg.ToolResults, 1)
	assert.Equal("call_aaaaaaaaaaaa", resultMsg.ToolResults[0].ToolUseID)
	assert.Positive(resultMsg.ToolResults[0].ContentLength)
}
