...
func TestParseClineSession_Full(t *testing.T) {
	...
	require.NoError(os.WriteFile(metaPath, []byte(metaJSON), 0o644))
	...
	require.NoError(os.WriteFile(messagesPath, []byte(messagesJSON), 0o644))

	sess, msgs, err := parseClineSession(metaPath, "astro-spectrometer", "local")
	require.NoError(err)
	require.NotNil(sess)

	assert.Equal("cline:"+sessionID, sess.ID)
	assert.Equal(AgentCline, sess.Agent)
	assert.Equal("Calibrate orbital telescope spectrometer frequency", sess.FirstMessage)
	assert.Equal("Calibrate orbital telescope spectrometer frequency", sess.SessionName)
	assert.Equal("astro_spectrometer", sess.Project)
	assert.Equal("/workspace/astro-spectrometer", sess.Cwd)
	assert.Equal("instruments/optics", sess.GitBranch)
	...
	assert.Equal(money.MustParseDollars("0.015"), *sess.UsageEvents[0].Cost)
	assert.Zero(sess.UsageEvents[0].InputTokens)
	assert.Zero(sess.UsageEvents[0].OutputTokens)
	assert.Zero(sess.UsageEvents[0].CacheReadInputTokens)
	assert.Zero(sess.UsageEvents[0].CacheCreationInputTokens)
	...
	assert.Equal("Calibrate orbital telescope spectrometer frequency", msgs[0].Content)
	...
	assert.Equal("I should read the optics definition and run tests.", msgs[1].ThinkingText)
	...
	assert.False(msgs[2].HasThinking)
	assert.True(msgs[2].HasToolUse)
	assert.Equal("I will inspect the optics instrumentation files.", msgs[2].Content)
	require.Len(msgs[2].ToolCalls, 2)

	tc0 := msgs[2].ToolCalls[0]
	assert.Equal("call_001", tc0.ToolUseID)
	assert.Equal("read_files", tc0.ToolName)
	assert.Equal("Read", tc0.Category)
	require.Len(tc0.ResultEvents, 1)
	assert.Equal("completed", tc0.ResultEvents[0].Status)
	assert.Contains(tc0.ResultEvents[0].Content, "class Spectrometer")

	tc1 := msgs[2].ToolCalls[1]
	assert.Equal("call_002", tc1.ToolUseID)
	assert.Equal("Skill", tc1.ToolName)
	assert.Equal("Tool", tc1.Category)
	assert.Equal("stellar-cartography", tc1.SkillName)
	require.Len(tc1.ResultEvents, 1)
	assert.Equal("completed", tc1.ResultEvents[0].Status)
	assert.Equal("Skill loaded successfully.", tc1.ResultEvents[0].Content)

	require.NotEmpty(msgs[2].TokenUsage)
	assert.JSONEq(`{"input_tokens":5000,"output_tokens":200,"cache_read_input_tokens":4000,"cache_creation_input_tokens":0}`, string(msgs[2].TokenUsage))
	assert.True(msgs[2].HasOutputTokens)
	assert.Equal(200, msgs[2].OutputTokens)
	assert.True(msgs[2].HasContextTokens)
	assert.Equal(9000, msgs[2].ContextTokens)
	...
	require.Len(msgs[3].ToolResults, 2)
	assert.Equal("call_001", msgs[3].ToolResults[0].ToolUseID)
	assert.Contains(DecodeContent(msgs[3].ToolResults[0].ContentRaw), "class Spectrometer")
	assert.Equal(len(tc0.ResultEvents[0].Content), msgs[3].ToolResults[0].ContentLength)

	// msg 4: final assistant text
	assert.Equal(RoleAssistant, msgs[4].Role)
	assert.Equal("Spectrometer optics calibrated to 432.0nm and telemetry frequency is locked.", msgs[4].Content)
	require.NotEmpty(msgs[4].TokenUsage)
	assert.JSONEq(`{"input_tokens":6000,"output_tokens":100,"cache_read_input_tokens":5000,"cache_creation_input_tokens":0}`, string(msgs[4].TokenUsage))
	assert.True(msgs[4].HasOutputTokens)
	assert.Equal(100, msgs[4].OutputTokens)
	assert.True(msgs[4].HasContextTokens)
	assert.Equal(11000, msgs[4].ContextTokens)
}
...
func TestParseClineSession_FallbackUsageEvent(t *testing.T) {
	...
	assert.Equal("cline:"+sessionID, ue.SessionID)
	assert.Equal("deepseek/deepseek-v4-flash", ue.Model)
	assert.Equal(20000, ue.InputTokens)
	assert.Equal(500, ue.OutputTokens)
	assert.Equal(15000, ue.CacheReadInputTokens)
	require.NotNil(ue.Cost)
	assert.Equal(money.MustParseDollars("0.015"), *ue.Cost)
}
...
func TestParseClineSession_CommandError(t *testing.T) {
	...
	require.NoError(os.WriteFile(filepath.Join(taskDir, sessionID+".json"), []byte(metaJSON), 0o644))
	...
	require.NoError(os.WriteFile(filepath.Join(taskDir, sessionID+".messages.json"), []byte(messagesJSON), 0o644))
	...
	assert.Contains(tc.ResultEvents[0].Content, "missing hydrophone header")
}
...
func TestParseClineSession_ThinkingContentInlining(t *testing.T) {
	...
	assert.Equal("Let me think about how to explain this.", asst.ThinkingText)
...
func TestParseClineSession_ThinkingOnlyEnding(t *testing.T) {
	...
	assert.Equal(TerminationToolCallPending, sess.TerminationStatus)
	assert.Equal("[Thinking]\nStill thinking...\n[/Thinking]", messages[1].Content)
	assert.Equal("Still thinking...", messages[1].ThinkingText)
	assert.True(messages[1].HasThinking)
}
...
func TestParseClineSession_ThinkingAndToolSeparated(t *testing.T) {
	...
	assert.True(messages[1].HasThinking)
	assert.False(messages[1].HasToolUse)
	assert.Equal("Checking repository state before executing tools.", messages[1].ThinkingText)
	assert.Equal("[Thinking]\nChecking repository state before executing tools.\n[/Thinking]", messages[1].Content)
	assert.Equal("m2:thinking", messages[1].SourceUUID)

	// Message 2: Tool call block (content is empty, ready for tool-group rendering)
	assert.Equal(RoleAssistant, messages[2].Role)
	assert.False(messages[2].HasThinking)
	assert.True(messages[2].HasToolUse)
	assert.Empty(messages[2].Content)
	assert.Equal("m2", messages[2].SourceUUID)
	require.Len(messages[2].ToolCalls, 1)
	assert.Equal("call_status_1", messages[2].ToolCalls[0].ToolUseID)
	assert.Equal("run_commands", messages[2].ToolCalls[0].ToolName)
}
...
func TestParseClineRawMessagesToolResultSurvivesMessageSliceGrowth(t *testing.T) {
	...
	require.Len(messages, 6)
	require.Len(messages[1].ToolCalls, 1)
	assert.Equal("call-1", messages[1].ToolCalls[0].ToolUseID)
	require.Len(messages[1].ToolCalls[0].ResultEvents, 1,
		"the result must resolve against the current parsed message slice")
	assert.Equal("completed", messages[1].ToolCalls[0].ResultEvents[0].Status)
	assert.Equal("completed output", messages[1].ToolCalls[0].ResultEvents[0].Content)
}
...
func TestParseClineSession_EmptyTranscript(t *testing.T) {
	...
	require.NotNil(sess)
	assert.Empty(msgs)
	assert.Equal("hello", sess.SessionName)
...
func TestParseClineSession_ZeroCostAuthoritative(t *testing.T) {
	...
	require.NoError(err)
	metaPath := filepath.Join(taskDir, sessionID+".json")
	require.NoError(os.WriteFile(metaPath, data, 0o644))
...
func TestParseClineSession_MessageMetricsZeroCostAuthoritative(t *testing.T) {
	...
	require.NotNil(sess)
	require.Len(msgs, 2)
	assert.Equal(200, sess.TotalOutputTokens)

	// Authoritative zero cost must be preserved on usage event with 0 tokens
	require.Len(sess.UsageEvents, 1)
	require.NotNil(sess.UsageEvents[0].Cost)
	assert.Equal(money.MustParseDollars("0"), *sess.UsageEvents[0].Cost)
	assert.Zero(sess.UsageEvents[0].InputTokens)
	assert.Zero(sess.UsageEvents[0].OutputTokens)
	assert.Zero(sess.UsageEvents[0].CacheReadInputTokens)
	assert.Zero(sess.UsageEvents[0].CacheCreationInputTokens)
...
func TestParseClineSession_ContentBlockToolResults(t *testing.T) {
	...
	tc := msgs[1].ToolCalls[0]
	assert.Equal("call_blk_1", tc.ToolUseID)
	require.Len(tc.ResultEvents, 1)
...
func TestParseClineSession_ProviderPropagation(t *testing.T) {
	...
	assert.Empty(msgs[0].ProviderID)

	// First assistant message should inherit session-level provider ("openrouter")
	assert.Equal("openrouter", msgs[1].ProviderID)
	assert.Equal("deepseek-chat", msgs[1].Model)

	// Second assistant message has per-message modelInfo overriding provider to "anthropic"
	assert.Equal("anthropic", msgs[2].ProviderID)
	assert.Equal("claude-3-5-sonnet", msgs[2].Model)

	// Aggregate usage event should carry session-level provider ("openrouter")
	require.NotEmpty(sess.UsageEvents)
	assert.Equal("openrouter", sess.UsageEvents[0].ProviderID)
	assert.Equal("deepseek-chat", sess.UsageEvents[0].Model)
}
...
func TestParseClineSession_TeammateSubagents(t *testing.T) {
	...
	require.NoError(os.WriteFile(filepath.Join(sessDir, "sess-parent.json"), []byte(metaJSON), 0o644))
	...
	require.NoError(os.WriteFile(filepath.Join(sessDir, "sess-parent.messages.json"), []byte(parentMsgsJSON), 0o644))
	...
	require.NoError(os.WriteFile(filepath.Join(sessDir, "git-scout__t1abc.messages.json"), []byte(teammateMsgsJSON), 0o644))
	...
	// Verify Parent
	parent := results[0]
	assert.Equal("cline:sess-parent", parent.Session.ID)
	assert.Empty(parent.Session.ParentSessionID)
	assert.Equal(RelNone, parent.Session.RelationshipType)
	assert.Equal("Lead Coordinator", parent.Session.SessionName)

	// Verify Tool Calls in Parent are annotated with child SubagentSessionID
	require.Len(parent.Messages, 5)
	spawnCall := parent.Messages[1].ToolCalls[0]
	assert.Empty(spawnCall.SubagentSessionID)
	assert.Equal("Task", spawnCall.Category)

	runCall := parent.Messages[3].ToolCalls[0]
	assert.Equal("cline:sess-parent__teamtask__git-scout__t1abc", runCall.SubagentSessionID)
	assert.Equal("Task", runCall.Category)
	require.Len(runCall.ResultEvents, 1)
	assert.Equal("cline:sess-parent__teamtask__git-scout__t1abc", runCall.ResultEvents[0].SubagentSessionID)
	assert.Equal("git-scout", runCall.ResultEvents[0].AgentID)

	// Verify Child Subagent
	child := results[1]
	assert.Equal("cline:sess-parent__teamtask__git-scout__t1abc", child.Session.ID)
	assert.Equal("sess-parent__teamtask__git-scout__t1abc", child.Session.SourceSessionID)
	assert.Equal("cline:sess-parent", child.Session.ParentSessionID)
	assert.Equal(RelSubagent, child.Session.RelationshipType)
	assert.Equal("Teammate: git-scout", child.Session.SessionName)
	assert.Equal("feat/team-feature", child.Session.GitBranch)
	assert.Equal("teamproject", child.Session.Project)
	assert.Equal("/workspace/teamproject", child.Session.Cwd)
	assert.Equal(AgentCline, child.Session.Agent)
	assert.Equal(3, child.Session.MessageCount)
	assert.Equal(1, child.Session.UserMessageCount)
	assert.Equal("Collect git status. taskId: task_0001", child.Session.FirstMessage)
	assert.Equal(80, child.Session.TotalOutputTokens)
	assert.Equal(250, child.Session.PeakContextTokens)

	// Verify child messages
	require.Len(child.Messages, 3)
	assert.True(child.Messages[1].HasThinking)
	assert.False(child.Messages[1].HasToolUse)
	assert.Equal("Executing git commands", child.Messages[1].ThinkingText)
	assert.False(child.Messages[2].HasThinking)
	assert.True(child.Messages[2].HasToolUse)
	require.Len(child.Messages[2].ToolCalls, 1)
	assert.Equal("run_commands", child.Messages[2].ToolCalls[0].ToolName)
	assert.Equal("Bash", child.Messages[2].ToolCalls[0].Category)
	...
	require.NoError(err)
	assert.Equal("cline:sess-parent", sess.ID)
	assert.Empty(msgs[1].ToolCalls[0].SubagentSessionID)
	assert.Equal("cline:sess-parent__teamtask__git-scout__t1abc", msgs[3].ToolCalls[0].SubagentSessionID)
}
...
func TestParseClineSession_AmbiguousTeammateRunsRemainUnlinked(t *testing.T) {
	...
	require.Len(results[0].Messages[0].ToolCalls, 1)
	call := results[0].Messages[0].ToolCalls[0]
	assert.Empty(call.SubagentSessionID)
	require.Len(call.ResultEvents, 1)
	assert.Empty(call.ResultEvents[0].SubagentSessionID)
	assert.Empty(call.ResultEvents[0].AgentID)
}
...
func TestParseClineTeammates_SafetyValidationAndSymlinks(t *testing.T) {
	...
	require.NoError(os.WriteFile(filepath.Join(sessDir, "sess-safe.json"), []byte(metaJSON), 0o644))
	...
	require.NoError(os.WriteFile(validPath, []byte(validJSON), 0o644))
	...
	require.NoError(os.WriteFile(filepath.Join(sessDir, "evil__1.messages.json"), []byte(badTeammate), 0o644))
	...
	require.NoError(os.WriteFile(filepath.Join(sessDir, "hidden__1.messages.json"), []byte(badUnderscore), 0o644))
	...
	require.NoError(os.WriteFile(filepath.Join(sessDir, "traversal__1.messages.json"), []byte(badPath), 0o644))
	...
	require.Len(results, 2)
	assert.Equal("cline:sess-safe", results[0].Session.ID)
	assert.Equal("cline:sess-safe__teamtask__good-scout__ok", results[1].Session.ID)
}
...
func TestParseClineSession_MultiTurnUserInputCleaning(t *testing.T) {
	...
	assert.Equal(5, sess.MessageCount)
	assert.Equal("Initial prompt from user", sess.FirstMessage)
	...
	assert.Equal("Initial prompt from user", msgs[0].Content)
	...
	assert.Equal("Now investigate the architecture", msgs[2].Content)
	...
	assert.Equal(RoleAssistant, msgs[4].Role)
	assert.Equal(4, msgs[4].Ordinal)
	assert.Equal("Proceeding with execution.", msgs[4].Content)
}
...
func TestParseClineTeammates_OneSessionPerFile(t *testing.T) {
	...
	require.NoError(os.WriteFile(filepath.Join(sessDir, "worker__aaa111.messages.json"), []byte(first), 0o644))
	require.NoError(os.WriteFile(filepath.Join(sessDir, "worker__bbb222.messages.json"), []byte(second), 0o644))
	require.NoError(os.WriteFile(filepath.Join(sessDir, "scout__ccc333.messages.json"), []byte(foreign), 0o644))

	results, err := parseClineSessionWithTeammates(filepath.Join(sessDir, "sess-runs.json"), "", "local")
	require.NoError(err)
	require.Len(results, 4, "parent plus one session per teammate file")

	byID := make(map[string]ParseResult)
	for _, r := range results[1:] {
		byID[r.Session.ID] = r
	}
	require.Contains(byID, "cline:sess-runs__teamtask__worker__aaa111")
	require.Contains(byID, "cline:sess-runs__teamtask__worker__bbb222")
	require.Contains(byID, "cline:sess-runs__teamtask__scout__ccc333")

	assert.Equal(2, byID["cline:sess-runs__teamtask__worker__aaa111"].Session.MessageCount)
	assert.Equal(4, byID["cline:sess-runs__teamtask__worker__bbb222"].Session.MessageCount)
	assert.Equal("other-parent__teamtask__scout__ccc333", byID["cline:sess-runs__teamtask__scout__ccc333"].Session.SourceSessionID)
	for _, r := range results[1:] {
		assert.Equal("cline:sess-runs", r.Session.ParentSessionID)
		assert.Equal(RelSubagent, r.Session.RelationshipType)
	}

	// Two worker files: the team_run_task call cannot pick one, so it stays unlinked.
	require.Len(results[0].Messages, 2)
	require.Len(results[0].Messages[0].ToolCalls, 1)
	assert.Empty(results[0].Messages[0].ToolCalls[0].SubagentSessionID)
}
