...
VALUES ('gap','gpt-5.4',100,?,'2026-09-08T12:00:03Z')`, tc.storeOutput)
			...
			require.NoError(err)
			require.NotNil(sess)
			assert.Equal(tc.wantTotal, sess.TotalOutputTokens)
			for _, msg := range msgs {
				assert.Empty(msg.TokenUsage, "no invented request-to-response correlation")
			}
			require.NotEmpty(events)
			assert.Equal(tc.storeOutput, events[0].OutputTokens)
			if tc.wantRemainder == 0 {
				require.Len(events, 1)
			} else {
				require.Len(events, 2)
				assert.Equal("transcript-output-remainder", events[1].Source)
				assert.Equal(tc.firstModel, events[1].Model)
				assert.Equal(tc.wantRemainder, events[1].OutputTokens)
				assert.Nil(events[1].MessageOrdinal)
			}
		})
	}
}
...
func TestParseCopilotSession_StoreUsageSupersedesShutdown(t *testing.T) {
	...
	assert.Equal("session-store", usage[0].Source)
	assert.Equal(80, usage[0].InputTokens)
	assert.Equal(50, usage[0].OutputTokens)
	assert.Nil(usage[0].Cost)
	assert.Empty(usage[0].CostSource)
	assert.Equal("2026-09-04T17:40:54.970Z", usage[0].OccurredAt)
	assert.Equal("session-store:42", usage[0].DedupKey)
	assert.Equal("shutdown", usage[1].Source)
	assert.Zero(usage[1].InputTokens)
	assert.Zero(usage[1].OutputTokens)
	assert.Zero(usage[1].CacheCreationInputTokens)
	assert.Zero(usage[1].CacheReadInputTokens)
	assert.Zero(usage[1].ReasoningTokens)
	require.NotNil(usage[1].Cost)
	assert.Equal(money.MustParseDollars("0.001"), *usage[1].Cost)
	assert.Equal(copilotReportedCostSource, usage[1].CostSource)
	assert.Equal("2026-09-04T17:00:03Z", usage[1].OccurredAt)
}
...
func TestLoadCopilotStoreUsage(t *testing.T) {
	...
	require.NoError(err)
	require.Len(events, 1)
	assert.Equal("session-store", events[0].Source)
	assert.Equal("claude-sonnet-4-6", events[0].Model)
	assert.Equal(30, events[0].InputTokens)
	assert.Equal(30, events[0].OutputTokens)
	assert.Equal(60, events[0].CacheReadInputTokens)
	assert.Equal(10, events[0].CacheCreationInputTokens)
	assert.Equal(4, events[0].ReasoningTokens)
	assert.Nil(events[0].Cost)
	assert.Empty(events[0].CostSource)
	assert.Equal("2026-09-04T17:40:54.970Z", events[0].OccurredAt)
	assert.Equal("session-store:7", events[0].DedupKey)
}
...
func TestParseCopilotSession_StoreWithoutUsageSchema(t *testing.T) {
	for _, shutdown := range []bool{false, true} {
		t.Run(fmt.Sprintf("shutdown=%t", shutdown), func(t *testing.T) {
			...
			if shutdown {
				require.Len(usage, 1)
				assert.Equal("shutdown", usage[0].Source)
				assert.Equal(10, usage[0].InputTokens)
				assert.Equal(3, usage[0].OutputTokens)
				assert.Empty(messages[0].TokenUsage)
			} else {
				assert.Empty(usage)
				assert.JSONEq(`{"output_tokens":3}`, string(messages[0].TokenUsage))
			}
		})
	}
}
...
func TestCopilotFingerprintRejectsUnreadableStoreState(t *testing.T) {
	for _, suffix := range []string{"", "-wal"} {
		t.Run("session-store.db"+suffix, func(t *testing.T) {
			...
			require.NoError(err, "a missing optional store is valid")
			...
			require.NoError(os.WriteFile(storePath+suffix, make([]byte, 100), 0o644))
			_, err = provider.Fingerprint(t.Context(), sources[0])
			assert.Error(err, "an unreadable marker must not become a valid fingerprint")

			if suffix == "" {
				require.NoError(os.WriteFile(storePath, original, 0o644))
			} else {
				require.NoError(os.Remove(storePath + suffix))
			}
			recovered, err := provider.Fingerprint(t.Context(), sources[0])
			require.NoError(err)
			assert.NotEqual(absent.Hash, recovered.Hash, "recovery must expose the available store")
		})
	}
}
