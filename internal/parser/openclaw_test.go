...
func TestParseOpenClawSession_Basic(t *testing.T) {
	...
	assert.Equal("openclaw:main:abc-123", sess.ID, "expected ID openclaw:main:abc-123, got %s")
	assert.Equal(AgentOpenClaw, sess.Agent, "expected agent openclaw, got %s")
	...
	require.Equal(2, len(msgs), "expected 2 messages, got %d")
...
func TestParseOpenClawSession_ToolResult(t *testing.T) {
	...
	require.NoError(err)
	require.Equal(4, len(msgs), "expected 4 messages, got %d")
	// Assistant with tool_use
	assert.True(msgs[1].HasToolUse, "expected HasToolUse=true for tool-use message")
	require.Equal(1, len(msgs[1].ToolCalls), "expected 1 tool call, got %d")
	...
	require.Equal(1, len(msgs[2].ToolResults), "expected 1 tool result, got %d")
...
func TestParseOpenClawSession_RealToolCallFormat(t *testing.T) {
	...
	assert.True(assistant.HasToolUse, "toolCall block must set HasToolUse")
	require.Equal(1, len(assistant.ToolCalls), "expected 1 parsed tool call")
	assert.Equal("bash", assistant.ToolCalls[0].ToolName)
	assert.Equal("call_1", assistant.ToolCalls[0].ToolUseID)
	assert.Contains(assistant.ToolCalls[0].InputJSON, "ls")

	// The standalone toolResult message pairs by toolCallId and
	// extracts the text from the toolResult content block.
	result := msgs[2]
	assert.Equal(RoleUser, result.Role)
	require.Equal(1, len(result.ToolResults), "expected 1 tool result")
	assert.Equal("call_1", result.ToolResults[0].ToolUseID)
	assert.Equal(len("file1\nfile2"), result.ToolResults[0].ContentLength,
	...
	require.NotEmpty(result.ToolResults[0].ContentRaw,
	...
	argsOnly := msgs[3]
	require.Equal(1, len(argsOnly.ToolCalls), "expected 1 parsed tool call")
	assert.Equal("read", argsOnly.ToolCalls[0].ToolName)
	assert.Contains(argsOnly.ToolCalls[0].InputJSON, "/etc/hosts")
}
...
func TestParseOpenClawSession_OrphanToolResult(t *testing.T) {
	...
	require.Equal(3, len(msgs), "expected 3 messages, got %d")
...
func TestParseOpenClawSession_AssistantUsage(t *testing.T) {
	...
	require.False(len(a.TokenUsage) == 0, "TokenUsage empty, want normalized JSON")
...
func TestParseOpenClawSession_AssistantUsageWithoutCost(t *testing.T) {
	...
	assert.False(len(a.TokenUsage) == 0, "TokenUsage empty, want normalized JSON")
...
func TestParseOpenClawSession_AgentIDInSessionID(t *testing.T) {
	...
	assert.Equal("openclaw:alpha:same-id", sessA.ID, "expected openclaw:alpha:same-id, got %s")
	assert.Equal("openclaw:beta:same-id", sessB.ID, "expected openclaw:beta:same-id, got %s")
}
...
func TestBestOpenClawEntry_CrossSuffix(t *testing.T) {
	...
	require.NoError(os.MkdirAll(sessDir, 0755))
	...
	require.Equal(1, len(files), "expected 1 (deduplicated), got %d")
...
func TestDiscoverOpenClawSessions(t *testing.T) {
	...
	require.NoError(os.MkdirAll(mainSessions, 0755))
	require.NoError(os.WriteFile(filepath.Join(mainSessions, "sess1.jsonl"), []byte("{}"), 0644))
	require.NoError(os.WriteFile(filepath.Join(mainSessions, "sessions.json"), []byte("{}"), 0644))

	claudeSessions := filepath.Join(root, "claude", "sessions")
	require.NoError(os.MkdirAll(claudeSessions, 0755))
	require.NoError(os.WriteFile(filepath.Join(claudeSessions, "sess2.jsonl"), []byte("{}"), 0644))

	files := discoverOpenClawSessionsForTest(t, root)
	require.Equal(2, len(files), "expected 2 session files, got %d")
	for _, f := range files {
...
func TestDiscoverOpenClawSessions_DeduplicatesArchived(t *testing.T) {
	...
	files := discoverOpenClawSessionsForTest(t, root)
	require.Equal(1, len(files), "expected 1 file (deduplicated), got %d")
...
func TestFindOpenClawSourceFile(t *testing.T) {
	...
	require.NoError(os.WriteFile(target, []byte("{}"), 0644))
	...
	assert.Equal("", notFound, "expected empty string, got %s")

	// Non-existent agent.
	notFound2 := findOpenClawSourceForTest(t, root, "other:abc-123")
	assert.Equal("", notFound2, "expected empty string, got %s")

	// Invalid format (no colon separator).
	notFound3 := findOpenClawSourceForTest(t, root, "abc-123")
	assert.Equal("", notFound3, "expected empty string for bare ID, got %s")
}
...
func TestFindOpenClawSourceFile_PrefersActiveOverArchive(t *testing.T) {
	...
	require.NoError(os.WriteFile(active, []byte("{}"), 0644))
