...
func TestClaudeSourceSetDiscoversS3Sessions(t *testing.T) {
	...
	src := sources[0]
	assert.Equal(AgentClaude, src.Provider)
...
func TestClaudeSourceSetMixedLocalAndS3Roots(t *testing.T) {
	...
	require.NoError(os.MkdirAll(localProj, 0o755))
	...
	for _, src := range sources {
		if _, ok := src.Opaque.(S3DiscoveredSource); ok {
			s3Count++
			assert.Equal(s3URI, src.DisplayPath)
		} else {
			localCount++
		}
	}
	assert.Equal(1, s3Count, "exactly one remote source")
	assert.Equal(1, localCount, "exactly one local source")
}
...
func TestCodexSourceSetDiscoversS3Sessions(t *testing.T) {
	...
	src := sources[0]
	assert.Equal(AgentCodex, src.Provider)
	assert.Equal(rolloutURI, src.DisplayPath)
	assert.Equal(rolloutURI, src.Key)

	s3, ok := src.Opaque.(S3DiscoveredSource)
	require.True(ok, "s3 source carries S3DiscoveredSource opaque")
	assert.Equal(rolloutURI, s3.URI)
	assert.Equal("coder", s3.Machine)
	assert.Equal(int64(11), s3.Size)
	assert.Equal(mtime.UnixNano(), s3.MtimeNS)
	assert.Contains(s3.Fingerprint, "rollout")
}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, root := range plan.Roots {
				...
				assert.False(strings.HasPrefix(root.Path, "s3:"), root.Path)
			}
			assert.True(foundLocal, "watch plan should retain the local root")
		})
	}
}
...
func TestCursorSourceSetDiscoversS3Sessions(t *testing.T) {
	...
	for _, src := range sources {
		...
		assert.Equal(AgentCursor, src.Provider)
		assert.Equal("demo-proj", src.ProjectHint)
		s3, ok := src.Opaque.(S3DiscoveredSource)
		require.True(ok)
		assert.Equal("laptop", s3.Machine)
		assert.Equal("demo-proj", s3.Project)
	}
	require.Contains(byPath, jsonlURI)
	require.Contains(byPath, txtURI)
	assert.Equal(int64(11), byPath[jsonlURI].Opaque.(S3DiscoveredSource).Size)
	assert.Equal(int64(7), byPath[txtURI].Opaque.(S3DiscoveredSource).Size)
}
...
func TestCursorSourceSetDiscoverEachYieldsS3Sessions(t *testing.T) {
	...
	require.NoError(err)
	require.Len(got, 1)
	assert.Equal(sessionURI, got[0].DisplayPath)
	assert.Equal(AgentCursor, got[0].Provider)
}
