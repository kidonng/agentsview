...
func TestCopilotStoreFingerprintWorkScalesWithChangedUsage(t *testing.T) {
	for _, count := range []int{8, 800} {
     ...
     VALUES (?, 'gpt-5.4', 100, 3, '2026-09-04T17:00:02Z')`, id, id)
			...
			for _, source := range sources {
				...
				require.NoError(err)
				assert.Equal(before[source.Key], fp)
			}
			assert.Zero(cold.sources.cache.transcriptBytes)
			provider = cold
    ...
    VALUES ('session-0000','gpt-5.4',100,7,'2026-09-04T17:00:03Z')`)
			...
			for _, source := range changed {
				...
				assert.Equal(before[source.Key].MTimeNS, fp.MTimeNS)
			}
			assert.Equal([]string{filepath.Join(root, copilotStateDir, "session-0000", "events.jsonl")}, changedKeys)
			assert.Zero(cache.transcriptBytes-bytesBefore, "a store-only event must not reread transcript payloads")
			assert.Equal(int64(2), cache.usageRows-rowsBefore, "only the changed session's two usage rows are read")
		})
	}
}
