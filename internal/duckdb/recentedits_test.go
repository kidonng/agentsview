...
func TestDuckRecentEditsSearchFilter(t *testing.T) {
	...
	require.NoError(err)
	require.Len(res.Files, 2, "matches Recent.go in both projects case-insensitively")
...
func TestDuckRecentEditsTieByCallIndex(t *testing.T) {
	...
	store := newDuckRecentEditsStore(t, func(local *db.DB) {
		...
		require.NoError(local.UpsertSession(s))
		...
		require.NoError(local.InsertMessages([]db.Message{msg}))
	})
