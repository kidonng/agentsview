...
func TestSyncPersistsSecretFindings(t *testing.T) {
	...
	require.NotZero(stats.Synced, "expected Synced > 0, got %+v", stats)

	sessionID := fx.sessionIDFor(t, path)

	sess, err := fx.db.GetSession(t.Context(), sessionID)
	require.NoError(err)
	require.NotNil(sess, "session %q not found after SyncAll", sessionID)
	assert.GreaterOrEqual(sess.SecretLeakCount, 1, "SecretLeakCount = %d, want >= 1", sess.SecretLeakCount)

	findings, err := fx.db.SessionSecretFindings(t.Context(), sessionID)
	require.NoError(err)
	require.NotEmpty(findings, "expected non-empty findings slice, got 0")
	...
	assert.True(sawAWS, "no aws-access-key finding in %+v", findings)
	// Pin the real use case: the secret captured in tool output must be
	// detected, not only the copy in the user message.
	assert.True(sawToolOutput, "no aws-access-key finding in tool output: %+v", findings)
}
...
func TestSyncNoSecretsLeavesZero(t *testing.T) {
	...
	require.NoError(err)
	require.NotNil(sess, "session %q not found after SyncAll", sessionID)
	assert.Equal(0, sess.SecretLeakCount)
	findings, err := fx.db.SessionSecretFindings(t.Context(), sessionID)
	require.NoError(err)
	assert.Empty(findings, "expected 0 findings, got %d: %+v", len(findings), findings)
}
