...
func TestDropPolicyNoResyncChurn(t *testing.T) {
	...
	require.NoError(database.ReplaceSessionMessages("resync", messages))

	engine := NewEngine(database, EngineConfig{})
	projected, _ := database.ProjectToolResultImages(messages)
	assert.NotContains(projected[0].ToolCalls[0].ResultContent, "input_image")
	assert.NotContains(projected[0].ToolCalls[0].ResultEvents[0].Content, "input_image")
	...
	require.NoError(engine.db.ReplaceSessionMessages("resync", projected))
	var after string
	require.NoError(database.Reader().QueryRowContext(t.Context(),
...
func TestPrepareSessionWritePreservesDecodedToolResults(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal("event summary", call.ResultContent)
			assert.Equal("Bash", call.ToolName)
			assert.Equal("Bash", call.Category)
		})
	}
}
...
func TestEngineImagePolicyOverridesDatabaseForBulkAppendAndLink(t *testing.T) {
	...
	require.Equal(1, bulk.writtenSessions)
	require.Equal(0, bulk.failedSessions)

	messages, err := database.GetAllMessages(t.Context(), bulkID)
	require.NoError(err)
	require.Len(messages, 1)
	require.Len(messages[0].ToolCalls, 1)
	assert.JSONEq(want, messages[0].ToolCalls[0].ResultContent)
	assert.Equal(len(want), messages[0].ToolCalls[0].ResultContentLength)
	...
	require.NoError(engine.writeMessages(bulkID, appendMessages))

	messages, err = database.GetAllMessages(t.Context(), bulkID)
	require.NoError(err)
	require.Len(messages, 2)
	assert.JSONEq(want, messages[1].ToolCalls[0].ResultContent)
	assert.Equal(len(want), messages[1].ToolCalls[0].ResultContentLength)
	...
	require.NoError(err)
	require.Len(messages, 1)
	assert.Equal(linkWant, messages[0].ToolCalls[0].ResultContent)
	assert.Equal(len(linkWant), messages[0].ToolCalls[0].ResultContentLength)
}
...
func TestEngineImagePolicyDeduplicatesLateProjectedResult(t *testing.T) {
	...
	assert.Empty(storedSummary)
	assert.JSONEq(want, storedEvent)
}
	...
	for _, mode := range []config.ToolResultImages{config.ToolResultImagesDrop, config.ToolResultImagesOffload} {
		t.Run(string(mode), func(t *testing.T) {
			...
			for _, id := range []string{"trashed", "source-missing"} {
				...
				require.NoError(writable.InsertMessages([]db.Message{{
			...
			require.NoError(writable.SoftDeleteSession("trashed"))
			require.NoError(writable.Update(func(tx *sql.Tx) error {
			...
			assert.NotContains(messages[0].ToolCalls[0].ResultEvents[0].Content, "input_image")
			for _, id := range []string{"trashed", "source-missing"} {
				...
				assert.NotContains(messages[0].ToolCalls[0].ResultEvents[0].Content,
					"input_image",
				)
				var storedEvent string
				require.NoError(replacement.Reader().QueryRowContext(
					t.Context(),
			 ...
			 WHERE session_id = ? AND tool_call_message_ordinal = ?
			   AND call_index = ?`, id, 0, 0,
				...
				if mode == config.ToolResultImagesOffload {
					assert.Contains(storedEvent, `"image_ref":"asset://`)
				}
			}
		})
	}
		...
		t.Run(string(mode), func(t *testing.T) {
			...
			for _, threshold := range []int64{1, 1 << 30} {
				t.Run(strconv.FormatInt(threshold, 10), func(t *testing.T) {
					...
					require.NoError(os.WriteFile(path, []byte(transcript), 0o600))
					...
					for _, late := range []bool{false, true} {
						...
						if mode == config.ToolResultImagesDrop {
							assert.JSONEq(want, calls[0].ResultContent)
						} else {
							assert.Contains(calls[0].ResultContent, `"image_ref":"asset://`)
						}
						count := 1
						if late {
							count = 2
						}
						require.Len(calls[0].ResultEvents, count)
						for _, event := range calls[0].ResultEvents {
							if mode == config.ToolResultImagesDrop {
								assert.JSONEq(want, event.Content)
							} else {
								assert.Contains(event.Content, `"image_ref":"asset://`)
							}
							assert.Equal(len(event.Content), event.ContentLength)
						}
					}
				})
			}
...
func TestCodexDropImagesNeverEnterScratch(t *testing.T) {
	...
	require.NoError(err)
	t.Cleanup(func() { require.NoError(sink.Close()) })
	...
	assert.Equal(len(summary), length)
	var content string
	var eventLength int
	require.NoError(sink.scratch.QueryRowContext(t.Context(), "SELECT content, content_length FROM stage_events LIMIT 1").Scan(&content, &eventLength))
	assert.JSONEq(want, content)
	assert.Equal(len(want), eventLength)
	bytes, err := os.ReadFile(sink.Path())
	require.NoError(err)
	assert.NotContains(string(bytes), "data:image/png;base64,AAEC")
}
...
func TestCodexDropImagesNeverEnterScratchWhenToolContentOmitted(t *testing.T) {
	...
	assert.JSONEq(want, content)
	assert.Equal(len(want), contentLength)
	bytes, err := os.ReadFile(sink.Path())
	require.NoError(err)
	assert.NotContains(string(bytes), "data:image/png;base64,AAEC")
}
...
func TestToolResultImagesOffloadFullIngest(t *testing.T) {
	...
	require.Len(messages[0].ToolCalls, 1)
	require.Len(messages[0].ToolCalls[0].ResultEvents, 1)
	require.Contains(messages[0].ToolCalls[0].ResultEvents[0].Content, `"image_ref":"asset://`)
	...
	for _, omitted := range []bool{false, true} {
		for _, blocked := range []bool{false, true} {
			t.Run(fmt.Sprintf("omitted=%t/blocked=%t", omitted, blocked), func(t *testing.T) {
				...
				require.NoError(sink.PublishToolResultImages())
				var content string
				var length int
				require.NoError(sink.scratch.QueryRowContext(t.Context(), "SELECT content, content_length FROM stage_events LIMIT 1").Scan(&content, &length))
				entries, err = os.ReadDir(database.AssetsDir())
				require.NoError(err)
				if blocked || omitted {
					assert.Empty(entries)
					...
					require.NoError(err)
					assert.Equal([]byte{0, 1, 2}, body)
					assert.Contains(content, `"image_ref":"asset://`)
					assert.Equal(len(content), length)
				}
				if omitted {
					assert.NotContains(content, "data:image/png;base64,AAEC")
					if !blocked {
						assert.Equal(len(content), length)
						assert.Contains(content, "agentsview_image")
						assert.NotContains(content, `"image_ref":"asset://`)
					}
				}
				if blocked {
					assert.Empty(content)
					assert.Equal(len(raw), length)
				}
			})
		}
	}
