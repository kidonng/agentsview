...
func TestOpenCodeV2ToolFilesSurviveSync(t *testing.T) {
 ...
 ('msg_files', 'ses_files', 'assistant', 1, 1700000000000, 1700000002000, ?)`, string(raw))
			...
			for i, call := range messages[0].ToolCalls {
				...
				assert.Equal(stored, call.ResultEvents[0].Content)
				assert.Equal(stored, call.ResultContent, "summary reads the same retained result")
				var blocks []map[string]any
				require.NoError(json.Unmarshal([]byte(stored), &blocks))
				if i == 0 {
					require.Len(blocks, 3)
					assert.Equal("Image read successfully", blocks[0]["text"])
					assert.Equal("After the image", blocks[2]["text"])
					if policy == config.ToolResultImagesKeep {
						assert.Equal("input_image", blocks[1]["type"])
						assert.Equal(imageURI, blocks[1]["image_url"])
					} else {
						assert.Equal("agentsview_image", blocks[1]["type"])
						assert.Equal(float64(68), blocks[1]["byte_size"])
						assert.Equal("plot.png", blocks[1]["name"])
						assert.NotContains(stored, imageURI)
					}
				} else {
					var files []map[string]string
					require.NoError(json.Unmarshal([]byte(stored), &files))
					assert.Equal(source.Content[1].State.Content, files, "image policy must retain PDFs and other file records")
					pdf, err := base64.StdEncoding.DecodeString(strings.TrimPrefix(files[1]["uri"], "data:application/pdf;base64,"))
					require.NoError(err)
					assert.Len(pdf, 327)
					assert.True(strings.HasPrefix(string(pdf), "%PDF-1.4\n"))
					text, err := base64.StdEncoding.DecodeString(strings.TrimPrefix(files[2]["uri"], "data:text/plain;base64,"))
					require.NoError(err)
					assert.Equal("alpha\n", string(text))
				}
			}
			assert.Zero(engine.SyncAll(t.Context(), nil).Synced, "unchanged payloads do not cause resync churn")
		})
	}
}
