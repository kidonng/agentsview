...
func TestDisabledProviderPreservesArchivedSession(t *testing.T) {
	...
	require.NoError(os.MkdirAll(filepath.Dir(newSource), 0o755))
	require.NoError(os.WriteFile(newSource, []byte(testjsonl.GeminiSessionJSON(
	...
	assert.Nil(stored.DeletionCause)
	assert.Equal(source, *stored.FilePath)
}
...
func TestDisabledProviderPreservesArchivedSessionAcrossRebuild(t *testing.T) {
	...
	require.NoError(os.MkdirAll(filepath.Dir(geminiSource), 0o755))
	require.NoError(os.WriteFile(geminiSource, []byte(`{"sessionId":"old"}`), 0o644))
	require.NoError(database.UpsertSession(db.Session{
	...
	require.False(stats.Aborted, "rebuild warnings: %v", stats.Warnings)

	stored, err := database.GetSessionFull(t.Context(), "archived-gemini-rebuild")
	require.NoError(err)
	require.NotNil(stored)
	assert.Nil(stored.DeletedAt)
	assert.Equal(geminiSource, *stored.FilePath)
	messages, err := database.GetMessages(
		t.Context(), "archived-gemini-rebuild", 0, 10, true,
	)
	require.NoError(err)
	require.Len(messages, 1)
	assert.Equal("preserve this archived message", messages[0].Content)
}
...
func TestDisabledCodebuffPreservesArchivedFreebuffAcrossRebuild(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NoError(os.MkdirAll(filepath.Dir(freebuffSource), 0o755))
			require.NoError(os.WriteFile(freebuffSource, []byte(`[]`), 0o644))
			require.NoError(database.UpsertSession(db.Session{
			...
			require.NoError(err)
			require.NotNil(stored)
			assert.Equal(string(parser.AgentFreebuff), stored.Agent)
			messages, err := database.GetMessages(
				t.Context(), stored.ID, 0, 10, true,
			)
			require.NoError(err)
			require.Len(messages, 1)
			assert.Equal("preserve this Freebuff message", messages[0].Content)
		})
	}
}
