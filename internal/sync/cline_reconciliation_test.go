...
func TestClineReconciliation_DeletedTeammateLifecycle(t *testing.T) {
	...
	require.NoError(os.WriteFile(msgPath, []byte(msgJSON), 0o644))
	...
	require.NoError(os.WriteFile(tmPath, []byte(tmJSON), 0o644))
...
func TestClineReconciliation_DeletedMetadataLifecycle(t *testing.T) {
	...
	for _, id := range []string{parentID, teammateID} {
		...
		assert.Nil(session.SourceMissingAt)
		assert.Nil(session.DeletedAt)
	}

	require.NoError(os.Remove(metaPath))
	require.NoError(engine.SyncPathsContext(t.Context(), []string{metaPath}))

	for _, id := range []string{parentID, teammateID} {
		session, err := database.GetSessionFull(t.Context(), id)
		require.NoError(err)
		require.NotNil(session, "session %s should remain archived", id)
...
func TestClineRemoteIdentityStableAcrossResyncs(t *testing.T) {
	...
	require.NoError(os.WriteFile(metaPath, []byte(`{"session_id":"sess-remote","cwd":"/workspace"}`), 0o644))
	...
	require.NoError(os.WriteFile(parentPath, []byte(parentJSON), 0o644))
	...
	require.NoError(os.WriteFile(teammatePath, []byte(teammateJSON("first")), 0o644))
	...
	first := engine.SyncAll(t.Context(), nil)
	require.Positive(first.Synced)
	wantParent, wantChild, wantLink, wantResultLink := readIdentity()
	assert.Equal(wantParent, parentID)
	assert.Equal(wantChild, childID)
	assert.Equal(wantLink, childID)
	assert.Equal(wantResultLink, childID)

	for _, text := range []string{"second", "third"} {
		require.NoError(os.WriteFile(teammatePath, []byte(teammateJSON(text)), 0o644))
		res := engine.SyncAll(t.Context(), nil)
		require.Positive(res.Synced)
		gotParent, gotChild, gotLink, gotResultLink := readIdentity()
		assert.Equal(wantParent, gotParent)
		assert.Equal(wantChild, gotChild)
		assert.Equal(wantLink, gotLink)
		assert.Equal(wantResultLink, gotResultLink)
	}

	storedPath := rewrite(teammatePath)
	ids, err := database.ListSessionIDsByFilePath(storedPath, string(parser.AgentCline))
	require.NoError(err)
	assert.Equal([]string{childID}, ids)
}
