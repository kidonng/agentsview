...
func TestCodexSidecarSignatureCoversAliasHomeIndexes(t *testing.T) {
	...
	require.NoError(os.WriteFile(primaryIndex, []byte("{}\n"), 0o600))
	before, latest, ok := codexSidecarSignature(session, metadata)
	require.True(ok)
	assert.Equal(before.sidecarMtime, latest)
	assert.Zero(before.sidecarAliases, "no alias index yet")
	assert.NotZero(before.sidecarMtime, "primary index is fingerprinted")

	aliasIndex := filepath.Join(alias, parser.CodexSessionIndexFilename)
	require.NoError(os.WriteFile(aliasIndex, []byte("{}\n"), 0o600))
	withAlias, _, ok := codexSidecarSignature(session, metadata)
	require.True(ok)
	assert.NotZero(withAlias.sidecarAliases)
	assert.Equal(before.sidecarMtime, withAlias.sidecarMtime,
		"primary fields do not change when an alias index appears")

	// A title written in the second home changes the signature.
	later := time.Now().Add(2 * time.Second)
	require.NoError(os.WriteFile(aliasIndex,
		[]byte(`{"id":"019f0000-0000-7000-8000-000000000001","thread_name":"renamed"}`+"\n"), 0o600))
	require.NoError(os.Chtimes(aliasIndex, later, later))
	renamed, latest, ok := codexSidecarSignature(session, metadata)
	require.True(ok)
	assert.NotEqual(withAlias.sidecarAliases, renamed.sidecarAliases)
	assert.Equal(withAlias.sidecarMtime, renamed.sidecarMtime)
	// The trusted mtime follows the newest index in any home, matching
	// what the archive stores through CodexEffectiveMtime.
	assert.Equal(later.UnixNano(), latest)
	assert.Equal(latest, metadata.EffectiveMtime(session, 0))
}
