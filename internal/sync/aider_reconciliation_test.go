...
func TestReconcileWatchRootsAiderOverlappingRootsReuseStableRunIdentity(t *testing.T) {
	...
	require.NoError(err)
	assert.NotNil(active, "the physical run keeps its parser-derived identity")
	...
	assert.Equal([]string{expectedID}, activeIDs,
		"overlapping roots must produce exactly one active Aider session")
	assert.Equal(1, engine.LastReconciliationResult().Metrics.SharedContainerScans)
}
...
func TestReconcileWatchRootsAiderScansOneLargeContainerOnce(t *testing.T) {
	...
	assert.Less(result.Metrics.MaxProviderRetainedBytes, int64(1<<20))
}
...
func TestReconcileWatchRootsAiderTombstonesDeletedVirtualRun(t *testing.T) {
	...
	secondRun := "# aider chat started at 2026-06-09 15:30:00\n" +
		"#### delete this run\ndeleted answer\n"
	require.NoError(os.WriteFile(historyPath, []byte(firstRun+secondRun), 0o644))
	...
	require.NoError(os.WriteFile(historyPath, []byte(firstRun), 0o644))
...
func TestReconcileWatchRootsAiderTombstonesDeletedRunWhenPositionIsReused(t *testing.T) {
	...
	require.NoError(os.WriteFile(historyPath, []byte(strings.Join(runs, "")), 0o644))
	deletedRawID, ok := parser.AiderRawIDAt(historyPath, 1)
	require.True(ok)
	...
	require.NoError(os.WriteFile(historyPath, []byte(runs[0]+runs[2]), 0o644))
	...
	require.NoError(err)
	require.NotNil(shifted)
	assert.Nil(shifted.DeletedAt)
	require.NotNil(shifted.FilePath)
	assert.Equal(parser.AiderVirtualPath(historyPath, 1), *shifted.FilePath)
}
