...
func TestCopilotBlockedResultPreservesExecutionTiming(t *testing.T) {
	...
	require.NoError(os.MkdirAll(filepath.Dir(eventsPath), 0o755))
...
`
	require.NoError(os.WriteFile(eventsPath, []byte(initialEvents), 0o644))
	...
	require.NoError(err)
	require.Len(beforeCompletion, 2)
	require.Len(beforeCompletion[1].ToolCalls, 1)
	require.Len(beforeCompletion[1].ToolCalls[0].ResultEvents, 1)
	assert.Equal("started", beforeCompletion[1].ToolCalls[0].ResultEvents[0].Status)
...
`
	...
	assert.Empty(call.ResultEvents[1].Content)
	assert.Equal("completed", call.ResultEvents[1].Status)
	assert.Equal("2026-04-26T10:00:04.825Z", call.ResultEvents[1].Timestamp)
