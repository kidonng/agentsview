...
func TestSourceBaselineSchemaUsesLocalCompanionTable(t *testing.T) {
	...
	assert.Equal(1, baselineTables)
	var indexSQL string
	require.NoError(d.getReader().QueryRow(`
		SELECT sql FROM sqlite_master
		WHERE type = 'index' AND name = 'idx_local_source_baselines_ownership'`,
	).Scan(&indexSQL))
	assert.Contains(strings.ToLower(strings.Join(strings.Fields(indexSQL), " ")),
...
func TestSessionPushEnumerationKeepsSourceStateLocal(t *testing.T) {
	...
	require.NoError(err)
	require.NotNil(full)
	require.NotNil(full.SourceMissingAt)
	...
	require.NotNil(sessions[0].SourceMissingAt)
	assert.Nil(sessions[0].DeletionCause)
}
