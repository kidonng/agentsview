...
func TestRecentEditsGroupingAndOrdering(t *testing.T) {
	...
	require.NoError(err)
	require.Len(res.Files, 2, "same relative path in two projects = 2 rows")
	// Most recent group first: projA (12:00) before projB (11:00).
	assert.Equal("projA", res.Files[0].Project)
	assert.Equal("config.go", res.Files[0].FilePath)
	assert.Equal(2, res.Files[0].EditCount)
	assert.Equal("projB", res.Files[1].Project)
	assert.Equal("config.go", res.Files[1].FilePath)
	assert.Equal(1, res.Files[1].EditCount)
...
func TestRecentEditsProjectFilter(t *testing.T) {
	...
	require.NoError(err)
	require.Len(res.Files, 1, "project filter should narrow to alpha only")
	assert.Equal("alpha", res.Files[0].Project)
	assert.Equal("a.go", res.Files[0].FilePath)
}
...
func TestRecentEditsSearchFilter(t *testing.T) {
	...
	require.NoError(err)
	require.Len(res.Files, 1, "search matches path substring case-insensitively")
	assert.Equal("internal/db/Recent.go", res.Files[0].FilePath)
	...
	assert.Equal("internal/server/handler.go", res.Files[0].FilePath)
	...
	require.NoError(err)
	require.Len(res.Files, 1, "escaped underscore matches literally, not as a wildcard")
	assert.Equal("x/a_b.go", res.Files[0].FilePath)
	...
	require.NoError(err)
	require.Len(res.Files, 1, "project and search both apply")
	assert.Equal("proj", res.Files[0].Project)
}
...
func TestRecentEditsTruncationAndHasMore(t *testing.T) {
	...
	require.NoError(err)
	assert.Len(res.Files, 2, "only Limit files should be returned")
	assert.True(res.HasMore, "HasMore should be true when more files exist")
	...
	for _, f := range res2.Files {
		if f.FilePath == "a.go" {
			assert.True(f.EditsTruncated, "a.go has 2 edits, MaxEditsPerFile=1 => truncated")
			assert.Len(f.Edits, 1, "only 1 edit inlined")
			found = true
		}
	}
	assert.True(found, "a.go should appear in result")
}
...
func TestRecentEditsNullTimestampsSortLast(t *testing.T) {
	...
	res, err := d.RecentEdits(ctx, RecentEditsParams{})
	require.NoError(err)
	require.Len(res.Files, 2, "both files should appear")
	// real.go (has timestamp) should sort before null.go (NULL timestamp).
	assert.Equal("real.go", res.Files[0].FilePath, "timestamped file first")
	assert.Equal("null.go", res.Files[1].FilePath, "null-timestamp file last")
}
...
func TestRecentEditsTieByCallIndex(t *testing.T) {
	...
	require.NoError(d.InsertMessages([]Message{msg}), "insert tie message")

	res, err := d.RecentEdits(ctx, RecentEditsParams{MaxEditsPerFile: 5})
	require.NoError(err)
	require.Len(res.Files, 1)
	require.Len(res.Files[0].Edits, 2, "both edits inlined")
	// rn=1 is the first in the Edits slice; it must be the higher call_index.
	assert.Equal(1, res.Files[0].Edits[0].CallIndex, "higher call_index ranks first")
	assert.Equal(0, res.Files[0].Edits[1].CallIndex)
}
...
func TestRecentEditsFileGroupTieByCallIndex(t *testing.T) {
	...
	require.NoError(d.InsertMessages([]Message{msg}), "insert group message")

	res, err := d.RecentEdits(ctx, RecentEditsParams{})
	require.NoError(err)
	require.Len(res.Files, 2)
	assert.Equal("aaa.go", res.Files[0].FilePath, "later call_index file first")
	assert.Equal("zzz.go", res.Files[1].FilePath)
}
