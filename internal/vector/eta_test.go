...
func TestBuildETAEstimatorPublishesSmoothedRateAfterWarmup(t *testing.T) {
	...
	assert.False(estimator.sample("embedding", 0, 1000, base).Ready)
	assert.False(estimator.sample("embedding", 100, 1000, base.Add(2*time.Second)).Ready)

	got := estimator.sample("embedding", 200, 1000, base.Add(4*time.Second))
	require.True(got.Ready)
	assert.InDelta(50, got.RatePerSecond, 0.001)
	assert.Equal(16*time.Second, got.Remaining)

	assert.True(estimator.sample("embedding", 200, 1000, base.Add(6*time.Second)).Ready,
		"an unchanged observation must retain the current estimate")
	got = estimator.sample("embedding", 300, 1000, base.Add(8*time.Second))
	require.True(got.Ready)
	assert.InDelta(42.5, got.RatePerSecond, 0.001,
		"the resumed delta must include the stalled interval in its instantaneous rate")
	assert.InDelta(time.Duration(16.470588*float64(time.Second)), got.Remaining,
...
func TestBuildETAEstimatorResetsDiscontinuousProgress(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.False(got.Ready)
			assert.Zero(got.RatePerSecond)
			assert.Zero(got.Remaining)
		})
	}
}
